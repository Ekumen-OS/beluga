...
class TestLifecycle : public ::testing::Test {
 public:
  void SetUp() override {
    rclcpp::init(0, nullptr);
    node_ = std::make_shared<beluga_amcl::NdtAmclNode3D>();
    node_->set_parameter(rclcpp::Parameter("map_path", kTestMapPath));
    node_->set_parameter(rclcpp::Parameter("scan_topic", "point_cloud"));
    node_->set_parameter(rclcpp::Parameter{"min_particles", 10});
    node_->set_parameter(rclcpp::Parameter{"max_particles", 10});
  }

  void TearDown() override { rclcpp::shutdown(); }

  std::shared_ptr<beluga_amcl::NdtAmclNode3D> node_ = nullptr;
};
...
TEST_F(TestLifecycle, AutoStart) {
  ...
  auto node = std::make_shared<beluga_amcl::NdtAmclNode3D>(rclcpp::NodeOptions{}
                                                               ...
                                                               .append_parameter_override("min_particles", 10)
                                                               .append_parameter_override("max_particles", 10));
...
TEST_F(TestNode, SetInitialPose) {
  ...
  ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.z", 2.0});
  ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.roll", 0.2});
  ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.pitch", 0.3});
  ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.yaw", 0.1});
...
TEST_F(TestNode, KeepCurrentEstimate) {
  ndt_amcl_node_->set_parameter(rclcpp::Parameter{"set_initial_pose", true});
  {
    ...
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.x", 34.0});
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.y", 2.0});
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.yaw", 0.0});
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.covariance_x", 0.001});
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.covariance_y", 0.001});
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.covariance_yaw", 0.001});
    ndt_amcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.covariance_xy", 0.0});
