...
class MclNodeUnderTest : public beluga_amcl::NdtMclNode {
 public:
...
class BaseNodeFixture : public T {
 public:
  void SetUp() override {
    ...
    ndt_mcl_node_ = std::make_shared<MclNodeUnderTest>();
 ...
 protected:
  std::shared_ptr<MclNodeUnderTest> ndt_mcl_node_;
...
TEST_F(TestLifecycle, FullSpin) {
  ...
  auto node = std::make_shared<beluga_amcl::NdtMclNode>();
...
TEST_F(TestLifecycle, AutoStart) {
  using lifecycle_msgs::msg::State;
  auto node = std::make_shared<beluga_amcl::NdtMclNode>(rclcpp::NodeOptions{}
...
TEST_P(TestInitializationWithModel, ParticleCount) {
  ...
  ndt_mcl_node_->set_parameter(rclcpp::Parameter{"num_particles", 30});
  ...
  std::visit([](const auto& pf) { ASSERT_EQ(pf.particles().size(), 30UL); }, *ndt_mcl_node_->particle_filter());
}
...
TEST_F(TestNode, NoBroadcastWhenInitialPoseInvalid) {
  ...
  ndt_mcl_node_->set_parameter(rclcpp::Parameter{"initial_pose.covariance_xy", -50.0});
  ndt_mcl_node_->configure();
  ndt_mcl_node_->activate();
  ASSERT_TRUE(wait_for_initialization());
  ASSERT_FALSE(tester_node_->can_transform("map", "odom"));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    TestParameterValue,
    testing::Values(
        rclcpp::Parameter("num_particles", -1),
...
TEST_P(TestParameterValue, InvalidValue) {
  ...
  ASSERT_ANY_THROW(std::make_shared<beluga_amcl::NdtMclNode>(options));
}
