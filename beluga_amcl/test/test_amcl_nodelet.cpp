...
public:
  ...
  : tf_listener_(tf_buffer_)
  {
    ...
    global_localization_client_ =
      nh_.serviceClient<std_srvs::Empty>("reinitialize_global_localization");
  }

  void create_pose_subscriber()
  {
    pose_subscriber_ = nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>(
      "pose", 10, boost::bind(&Tester::pose_callback, this, _1));
  }
  ...
  void create_particle_cloud_subscriber()
  {
    particle_cloud_subscriber_ = nh_.subscribe<geometry_msgs::PoseArray>(
      "particle_cloud", 10, boost::bind(&Tester::particle_cloud_callback, this, _1));
  }
...
public:
  ...
  {
    return spin_until([this] {return amcl_nodelet->is_initialized();}, 200ms);
  }

  bool wait_for_pose_estimate()
  {
    return spin_until(
      [this] {return tester->latest_pose().has_value();}, 100ms);
  }
...
TEST_P(TestInitializationWithModel, ParticleCount)
{
  ...
  config.robot_model_type = motion_model;
...
TEST_F(TestFixture, SetInitialPose)
{
  ...
  config.initial_pose_yaw = 0.3;
  config.initial_pose_covariance_x = 0.001;
  config.initial_pose_covariance_y = 0.001;
  config.initial_pose_covariance_yaw = 0.001;
  config.initial_pose_covariance_xy = 0.0;
  config.initial_pose_covariance_xyaw = 0.0;
  config.initial_pose_covariance_yyaw = 0.0;
  ASSERT_TRUE(amcl_nodelet->set(config));
...
TEST_F(TestFixture, NoBroadcastWhenInitialPoseInvalid)
{
  ...
  ASSERT_TRUE(amcl_nodelet->default_(config));
  config.set_initial_pose = true;
  config.initial_pose_covariance_x = 0.0;
  config.initial_pose_covariance_y = 0.0;
  config.initial_pose_covariance_xy = -50.0;
  ASSERT_TRUE(amcl_nodelet->set(config));
...
TEST_F(TestFixture, InvalidMotionModel)
{
  beluga_amcl::AmclConfig config;
  ASSERT_TRUE(amcl_nodelet->default_(config));
  config.robot_model_type = "non_existing_model";
  ASSERT_TRUE(amcl_nodelet->set(config));
  tester->publish_map();
