...
: Which observation model to use. Supported models are `beam`, `likelihood_field` and `likelihood_field_prob` as described in {cite}`thrun2005probabilistic` with the same aggregation formula used in Nav2 AMCL.
...
| `model_unknown_space` | Whether to model unknown space or assume it free. Only applicable to the `likelihood_field` model. Default is `false`. |  | ✅ |  |
| `only_obstacle_boundaries` | Whether to treat inner obstacle cells as unknown space, particularly when calculating likelihood fields. Only applicable to the `likelihood_field` model. Default is `true`. |  | ✅ |  |
