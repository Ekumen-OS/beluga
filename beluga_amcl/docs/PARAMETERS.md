...
This page describes the parameters supported by Beluga AMCL including a comparison table with Nav2 AMCL.
...
| `initial_pose_topic` | The name of the topic where an initial pose can be published. _The `initialpose` topic in Nav2 AMCL can be [remapped externally](https://design.ros2.org/articles/static_remapping.html)._ | | ✅ |
...
| `first_map_only` | Whether to accept maps more than once on the `map` topic. | ✅ | ✅ |
...
| `spatial_resolution_[x, y, theta]` | Spatial resolution to create buckets for KLD resampling [[1]](#1). | | ✅ |
...
| Parameter | Description | Navigation 2 AMCL | Beluga AMCL |
|-----------|-------------|:-----------------:|:-----------:|
| `robot_model_type` | Which odometry motion model to use. Supported models are `differential_drive` [[3]](#3), `omnidirectional_drive` and `stationary`. _Beluga AMCL also supports Nav2 AMCL plugin names (`nav2_amcl::DifferentialMotionModel`, `nav2_amcl::OmniMotionModel`) but it will load the equivalent Beluga model._ | ✅ | ✅ |
