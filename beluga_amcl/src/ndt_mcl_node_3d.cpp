...
NdtMclNode3D::NdtMclNode3D(const rclcpp::NodeOptions& options) : BaseMCLNode{"ndt_mcl", "", options} {
...
void NdtMclNode3D::do_activate(const rclcpp_lifecycle::State&) {
  ...
  particle_filter_ = make_particle_filter();
  {
    ...
    laser_scan_connection_ =
        laser_scan_filter_->registerCallback(std::bind(&NdtMclNode3D::laser_callback, this, std::placeholders::_1));
...
void NdtMclNode3D::do_deactivate(const rclcpp_lifecycle::State&) {
...
void NdtMclNode3D::do_cleanup(const rclcpp_lifecycle::State&) {
  RCLCPP_INFO(get_logger(), "Cleaning up");
  particle_cloud_pub_.reset();
  pose_pub_.reset();
...
auto NdtMclNode3D::get_initial_estimate() const -> std::optional<std::pair<Sophus::SE3d, Sophus::Matrix6d>> {
...
auto NdtMclNode3D::get_motion_model() const -> MotionModelVariant {
...
beluga::NDTSensorModel<NDTMapRepresentation> NdtMclNode3D::get_sensor_model() const {
...
auto NdtMclNode3D::make_particle_filter() const -> std::unique_ptr<NdtMcl3DVariant> {
  auto amcl = std::visit(
...
void NdtMclNode3D::do_initial_pose_callback(geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr message) {
...
void NdtMclNode3D::do_periodic_timer_callback() {
  if (!particle_filter_) {
    return;
  }
...
void NdtMclNode3D::laser_callback(sensor_msgs::msg::PointCloud2::ConstSharedPtr laser_scan) {
...
bool NdtMclNode3D::initialize_from_estimate(const std::pair<Sophus::SE3d, Sophus::Matrix6d>& estimate) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(beluga_amcl::NdtMclNode3D)
