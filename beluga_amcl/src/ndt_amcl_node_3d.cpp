...
NdtAmclNode3D::NdtAmclNode3D(const rclcpp::NodeOptions& options) : BaseAMCLNode{"ndt_amcl", "", options} {
...

  {
    ...
    this->declare_parameter("initial_pose.covariance_z", 0.0, descriptor);
  }

  {
    auto descriptor = rcl_interfaces::msg::ParameterDescriptor();
    descriptor.description = "Initial pose roll axis covariance.";
    this->declare_parameter("initial_pose.covariance_roll", 0.0, descriptor);
  }

  {
    auto descriptor = rcl_interfaces::msg::ParameterDescriptor();
    descriptor.description = "Initial pose pitch axis covariance.";
    this->declare_parameter("initial_pose.covariance_pitch", 0.0, descriptor);
  }
...
bool NdtAmclNode3D::initialize_from_estimate(const std::pair<Sophus::SE3d, Sophus::Matrix6d>& estimate) {
  ...
  RCLCPP_INFO(
      get_logger(), "Particle filter initialized with %ld particles about initial pose x=%g, y=%g", num_particles,
