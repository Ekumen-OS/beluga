...
class PoseSensorModel {
 ...
 public:
   ...
   */
  explicit PoseSensorModel(param_type params) : params_{std::move(params)} { assert(params_.min_weight > 0.0); }
   ...
   */
  [[nodiscard]] std::function<weight_type(const state_type&)> operator()(measurement_type&& measurement) const {
    const auto min_weight = params_.min_weight;
    // efficiently obtain the covariance matrix inverse using
    // Cholesky decomposition, exploiting the fact that the
    // covariance matrix is positive-definite
    const auto llt = measurement.covariance.llt();
    if (llt.info() != Eigen::ComputationInfo::Success) {
      // gracefully fail if the covariance matrix is not
      // positive-definite for some reason
      return [min_weight]([[maybe_unused]] const state_type& state) -> weight_type { return min_weight; };
    }
    const CovMatrix information_matrix = llt.solve(CovMatrix::Identity());
    const state_type pose_inv = measurement.pose.inverse();
    // return a weighting function that computes the error
    // between each particle state and the measurement,
    // and assigns a weight to each particle according to the
    // uncertainty of the measurement
    return [min_weight, pose_inv = std::move(pose_inv), information_matrix](const state_type& state) -> weight_type {
      const auto error = (pose_inv * state).log();
      return std::max(std::exp(-0.5 * error.dot(information_matrix * error)), min_weight);
    };
  }
