 ...
 private:
  ...
  static ValueGrid2<float> make_likelihood_field(const LikelihoodFieldModelParam& params, const OccupancyGrid& grid) {
    ...
    if (params.model_unknown_space) {
      ...
      const auto background_distance = -two_squared_sigma * std::log((inverse_max_distance - offset) / amplitude);

      distance_map |= beluga::actions::overlay(
          grid.unknown_mask(), std::min(squared_max_distance, static_cast<float>(background_distance)));
    }
