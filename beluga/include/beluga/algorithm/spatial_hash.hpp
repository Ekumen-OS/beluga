...
/**
 * Specialization for Sophus::SE3d. Will calculate the spatial hash based on the translation and rotation (expressed in
 * Euler Angles).
 */
template <>
class spatial_hash<Sophus::SE3d, void> {
 public:
  ...
  std::size_t operator()(const Sophus::SE3d& state) const {
    const auto& position = state.translation();
    // This gives yaw, pitch, roll in that order.
