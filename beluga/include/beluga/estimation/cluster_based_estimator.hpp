...
class ClusterBasedStateEstimator : public Mixin {
 public:
   ...
   */
  template <class... Args>
  explicit ClusterBasedStateEstimator(const param_type& parameters, Args&&... args)
      : Mixin(std::forward<Args>(args)...), parameters_{parameters} {}
...
std::pair<StateType, CovarianceType> ClusterBasedStateEstimator<Mixin, StateType, CovarianceType>::estimate() const {
  auto hashes =
      this->self().states() | ranges::views::transform(spatial_hash_function_) | ranges::to<std::vector<std::size_t>>();
  ...
  const auto weight_cap =
      cse_detail::calculate_percentile_weight_threshold(grid_cell_data, parameters_.weight_cap_percentile);
  ...
  auto per_cluster_estimates = estimate_clusters(this->self().states(), this->self().weights(), clusters);
