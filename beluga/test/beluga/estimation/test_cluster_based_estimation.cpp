...
TEST_F(ClusterBasedEstimationDetailTesting, MapGridCellsToClustersStep) {
  const double kFieldSide = 36.0;
  const double kHalfFieldSide = 18.0;

  // create a map with four independent peaks
  std::vector<std::tuple<double, double, double>> coordinates;
  for (double x = 0.0; x < kFieldSide; x += 1.0) {
    for (double y = 0.0; y < kFieldSide; y += 1.0) {
  ...
  const auto right_side_cell = [&](const auto& c) { return std::get<0>(c) >= kHalfFieldSide; };
  const auto left_side_cell = [&](const auto& c) { return !right_side_cell(c); };
  const auto top_side_cell = [&](const auto& c) { return std::get<1>(c) >= kHalfFieldSide; };
...
TEST_F(ClusterBasedEstimationDetailTesting, ClusterStatEstimationStep) {
  ...
  // create a map with four independent peaks
  auto [states, weights] = make_particle_multicluster_dataset(0.0, kFieldSide, 0.0, kFieldSide, 1.0);
...
TEST_F(ClusterBasedEstimationDetailTesting, ClusterBasedEstimationMixinTest) {
  ...
  const auto uut = UUT{ClusterBasedStateEstimatorParam{}};
