...
Eigen::Matrix3d make_diagonal_covariance_2d(
  ...
  return Eigen::DiagonalMatrix<double, 3>{sigma_x * sigma_x, sigma_y * sigma_y, sigma_theta * sigma_theta};
}
...
Eigen::Matrix<double, 6, 6> make_diagonal_covariance_3d(
  ...
  cov.diagonal() << sigma_trans * sigma_trans, sigma_trans * sigma_trans, sigma_trans * sigma_trans,
      sigma_rot * sigma_rot, sigma_rot * sigma_rot, sigma_rot * sigma_rot;
  return cov;
}
...
TYPED_TEST(PoseSensorModelTests, NonIdentityMeasurementBullsEye) {
  ...
  if constexpr (std::is_same_v<TypeParam, Sensor2D>) {
    meas = {
        Sophus::SE2d{Sophus::SO2d{Sophus::Constants<double>::pi() / 3.0}, Eigen::Vector2d{2.0, 5.0}},
        make_diagonal_covariance_2d()};
  } else {
    meas = {
        Sophus::SE3d{Sophus::SO3d::rotZ(Sophus::Constants<double>::pi() / 3.0), Eigen::Vector3d{2.0, 5.0, 0.0}},
