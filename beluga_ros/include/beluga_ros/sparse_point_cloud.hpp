...
/// \param StrictTypeCheck [optional]: If 'true' XYZ datafields must be the same type (float or double).
/// Other datafields can be different types.
template <typename T, bool StrictTypeCheck = true>
class SparsePointCloud3 : public beluga::BaseSparsePointCloud<SparsePointCloud3<T>> {
 public:
  ...
  explicit SparsePointCloud3(beluga_ros::msg::PointCloud2ConstSharedPtr cloud, Sophus::SE3d origin = Sophus::SE3d())
    ...
    // Check XYZ datatype is the same (for strict type check)
    if constexpr (StrictTypeCheck) {
  ...
  [[nodiscard]] auto points() const {
    const uint8_t cloud_type = cloud_->fields.at(0).datatype;
    constexpr uint8_t templetized_type = sensor_msgs::typeAsPointFieldType<T>::value;

    if constexpr (!StrictTypeCheck) {
      // Allow conversion from FLOAT64 → float
      if (templetized_type == sensor_msgs::msg::PointField::FLOAT32 &&
          cloud_type == sensor_msgs::msg::PointField::FLOAT64) {
        // Read as double, cast to float
        beluga_ros::msg::PointCloud2ConstIterator<double> iter_d(*cloud_, "x");
        return ranges::views::iota(0, static_cast<int>(cloud_->width * cloud_->height)) |
               ranges::views::transform([iter_d](int i) mutable {
                 Eigen::Vector3f point = Eigen::Map<const Eigen::Vector3d>(&(iter_d + i)[0]).cast<float>();
                 return point;
               });
      }
    }
